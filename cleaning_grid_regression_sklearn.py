...
def mask_cluster(df, eps,  cluster_id): 

	# reconstruct the column name
	col_name = 'cluster_M' + '_eps_' + str(eps)
	print col_name
...
def do_regression(eq_df, welldf, intervals, lock ,cv = 5, standardization = None):
	...
	for interval in intervals:
		...
		do_regression(X_prior,X_post,Y_prior,Y_post, lock ,cv , standardization)

	return

def do_regression(X_prior,X_post,Y_prior,Y_post, lock ,cv = 5, standardization = None):
	# ------------------------------------------
